    ...
    pullRequests
      .map((pullRequest) =>
        template(config['change-template'], {
          $TITLE: buildPRTitleWithLinks(pullRequest.title, config),
...
const buildPRTitleWithLinks = (title, config) => {
  const titleTemplateRegex =  new RegExp(config['task-number-template'])
  const urlPrefix = config['task-prefix']
  const titleWihtLinks = title.replace(titleTemplateRegex, `<a href="${urlPrefix}$1" target="_blank">$1</a>`)

  return titleWihtLinks
}
